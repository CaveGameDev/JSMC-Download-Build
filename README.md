...
```html
<script src="https://jsmc-download-site.netlify.app/embed.js"></script>
```
...
- **Demo**: `https://jsmc-download-site.netlify.app/demo.html` - Shows how to use the embed script
- **Integration Example**: `https://jsmc-download-site.netlify.app/integration-example.html` - Shows how to integrate on another website
...
WebsiteDownloader.config.apiUrl = 'https://jsmc-download-site.netlify.app';
...
1. **Netlify Functions**: API endpoints are handled by serverless functions in `/netlify/functions/`
2. **CORS Configuration**: Proper CORS headers are set in `netlify.toml`
3. **Static Files**: All static files are served from the `/public/` directory

### Deployment Steps:

1. **Push to Git**: Connect your repository to Netlify
2. **Build Settings**: 
...
3. **Use a database** instead of in-memory storage for download tracking
4. **Add HTTPS** for secure communication (handled by Netlify)
5. **Configure proper CORS** policies for your domain
